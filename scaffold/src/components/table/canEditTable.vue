            ...
            on: {
                'click': () => {
                    if (!currentRow.editting) {
                        ...
                        let message = '';
                        for (let key in edittingRow){
                            for (let item of vm.columnsList){
                                if (key == item.key && edittingRow[key] == ''){
                ...
                this.columnsList.forEach(item => {
                    if (item.editable) {
                        item.render = (h, param) => {
                            ...
                            const value = currentRow[item.key];
                            if (!currentRow.editting) {
                                if (this.editIncell) {
                                    return h('Row', {
                                        props: {
                                ...
                                if (elementType == '' || elementType == undefined){
                                    ...
                                    const max = item.max || 2147483648 ;
                                    const min = item.min || -2147483648;
